...
PATCHES=(
   "${FILESDIR}"/libcamera-9999-debayer-disable-bgr.patch
   # "${FILESDIR}"/libcamera-9999-softisp-adjust-yaml.patch
   # "${FILESDIR}"/libcamera-9999-softisp-agc-yaml.patch
   # "${FILESDIR}"/libcamera-9999-softisp-awb-yaml.patch
   # "${FILESDIR}"/libcamera-9999-softisp-context-float.patch
)
...
src_configure() {
	local emesonargs=(
		...
		$(meson_use test) -Dqcam=disabled -Dsoftisp-gpu=disabled
	)
	meson_src_configure
}
